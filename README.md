...
| 菜单项 / 子命令 | 功能 |
| --- | --- |
| `[1]` / `DSH Manager.bat start` | **一键启动**：检测 `http://127.0.0.1:3080` 是否已运行；未运行则自动执行启动命令并等待服务就绪（默认最多 600 秒，可用 `-TimeoutS` 调整），随后在浏览器中打开 GUI |
...
| `[4]` / `DSH Manager.bat uninstall` | **一键卸载**：停止服务器 → 卸载 npm 全局包（若存在）→ 删除桌面快捷方式与安装目录 → 清理 npx 缓存 |
| `[5]` | 退出 |
